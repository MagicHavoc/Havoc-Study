...
struct queue_entry {
  ...
  u8  cal_failed,                     /* Calibration failed?              */
      ...
      from_socket;                    /* The fname is from the socket?    */   
...
static void cull_queue(void) {
  ...
  while (q) {
    if (!q->from_socket) mark_as_redundant(q, !q->favored);
    q = q->next;
  }
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+bq:rhi:o:f:m:p:l:w:t:T:dnCB:S:M:x:Qsc")) > 0)
    ...
    switch (opt) {
      ...
      case 'l': /* time limit */
        set_time_limit(atoi(optarg));
        break;

      case 'w': /* iteration */
        set_iteration(atoi(optarg));
        break;
...
void handle_testcase(char** argv, char* fname) {
  ...
  if (st.st_size && st.st_size <= MAX_FILE) {
    ...
    u8* in_buf = mmap(0, st.st_size, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
    if (in_buf == MAP_FAILED) PFATAL("Unable to mmap '%s'", fname);

    u8* new_fpath = (u8*)malloc(MAX_LINE), *relative_path;
    memset(new_fpath, 0, MAX_LINE);

    if((relative_path = strstr(fname, "/queue/")) == NULL) {
      printf("File name must has '/queue/' !\n");
      exit(0);
    }

    strcat(new_fpath, out_dir);
    if(out_dir[strlen(out_dir) - 1] == '/') {
      strcat(new_fpath, relative_path + 1);
    } else {
      strcat(new_fpath, relative_path);
    }

    fd_new = open(new_fpath, O_WRONLY | O_CREAT | O_EXCL, 0600);
    ck_write(fd_new, in_buf, st.st_size, new_fpath);
    
    close(fd_new);
    munmap(in_buf, st.st_size);

    add_to_queue(new_fpath, st.st_size, 0, 1);
  }
  close(fd);
}
