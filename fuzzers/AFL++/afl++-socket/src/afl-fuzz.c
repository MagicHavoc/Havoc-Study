...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while ((opt = getopt(
              argc, argv,
              "+b:B:c:CdDe:E:gGhi:I:f:F:l:L:m:M:nNOo:p:P:RQs:S:t:T:UV:w:Wx:y:Z")) > 0) {
...
void handle_testcase(afl_state_t *afl, char* fname) {
  ...
  if (st.st_size && st.st_size <= MAX_FILE) {
    ...
    strcat(new_fpath, afl->out_dir);
    if(afl->out_dir[strlen(afl->out_dir) - 1] == '/') {
    ...
    add_to_queue(afl, new_fpath, st.st_size, 0, 1);
  }
  close(fd);
}
